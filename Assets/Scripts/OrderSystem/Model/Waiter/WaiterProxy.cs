...
namespace OrderSystem
{
    public class WaiterProxy : Proxy
    {
        ...
        public Queue<Order> WaitforWaiterOrder = new Queue<Order>();
        ...
        public WaiterProxy() : base(NAME,new List<WaiterItem>())
        {
            ...
            //AddWaiter(new WaiterItem(4 , "小明" , 0));
            //AddWaiter(new WaiterItem(5 , "小龙" , 0));
        }
        ...
        public void RemoveWaiter( WaiterItem item ) 
        {
           ...
           for (int i = 0; i < Waiters.Count; i++)
           {
               ...
               {
                    Waiters[i].state = 0;
                    if (WaitforWaiterOrder.Count>0)
                    {
                        Order order = WaitforWaiterOrder.Dequeue();
                        ChangeWaiterState(order);
                    }
                    else
                    {
        ...
        /// 查找空闲状态的服务员
        /// </summary>
        public void ChangeWaiterState(Order order)
        {
            ...
            {
                WaitforWaiterOrder.Enqueue(order);
            }
