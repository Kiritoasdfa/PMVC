        ...
        public override void HandleNotification( INotification notification )
        {
            switch (notification.Name)
            {
                ...
                case OrderSystemEvent.SUBMITMENU:
                    Order selectedOrder = notification.Body as Order;//生成订单
                    MenuView.SubmitMenu(selectedOrder);//看订单里勾选了哪些菜
                    //selectedOrder.client.state++;
                    SendNotification(OrderSystemEvent.ORDER, selectedOrder);//发送点菜命令
                    break;
